...
- `cd voice-api-inbound-video-room`
- `npm install`
...
- Make a file with name ".env" in root directory . And copy content of .env.example in .env file . Then set the environment variables manually in .env file. And below are the environment variables.
  - `ENABLEX_APP_ID` , `ENABLEX_APP_KEY` , `SERVICE_PORT` , `USE_PUBLIC_WEBHOOK` , `PUBLIC_WEBHOOK_HOST` , `USE_NGROK_TUNNEL` , `VIDEO_ROOMID` , `LISTEN_SSL`
...
- For Inbound Calls
  - `node client-inbound-video-room.js`
